...
function computeLowestTenAverage(twoRoundScores) {
  if (!Array.isArray(twoRoundScores) || twoRoundScores.length < 10) {
    return null;
  }

  const lowestTen = twoRoundScores
    .filter((score) => typeof score === 'number' && !Number.isNaN(score))
    .sort((a, b) => a - b)
    .slice(0, 10);

  if (lowestTen.length < 10) {
    return null;
  }

  const average = lowestTen.reduce((sum, score) => sum + score, 0) / lowestTen.length;
  return normalizeScoreNumber(average);
}
...
function resolveScoreForLookupNames(majorWithScores, lookupNames) {
  ...
  for (const lookupName of lookupNames) {
    ...
    if (majorWithScores.participantsByName?.has(lookupName)) {
      isParticipant = true;
    }
  }

  if (!isParticipant && typeof majorWithScores.defaultMissingScore === 'number') {
    return majorWithScores.defaultMissingScore;
  }
...
async function fetchMajorScoreMap(majorEvent) {
  ...
  const defaultMissingScore = secondRoundComplete ? computeLowestTenAverage(twoRoundScores) : null;
...
async function buildPayload(seasonYear, leagueName) {
  ...
  return {
    ...
    notes: {
      ...
      nonParticipantRule: 'After round 2 is complete, players not in the field are assigned the average of the 10 best two-round totals.'
    }
  };
}
